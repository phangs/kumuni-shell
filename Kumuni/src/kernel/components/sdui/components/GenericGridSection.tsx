...
const GenericGridSection: React.FC<SDUIRenderingProps> = ({ data, onAction }) => {
    return (
        <View style={styles.sectionContainer}>
            <View style={[styles.rowBetween, { alignItems: 'flex-start' }]}>
                ...
                {data.props.showMoreLabel && (
                    <TouchableOpacity onPress={() => onAction?.(data.props.showMoreAction || 'show_more')}>
                        <Text style={styles.linkText}>{data.props.showMoreLabel} {'>'}</Text>
                    </TouchableOpacity>
                )}
            </View>
            ...
            <View style={styles.bizGrid}>
                ...
                {data.props.items?.[2] && (
                    <TouchableOpacity style={styles.bizCardLargeVertical} onPress={() => onAction?.(data.props.items[2].action)}>
                        <View style={styles.cardImageContainerLarge}><Image source={{ uri: data.props.items[2].imageUrl }} style={styles.bizImgLarge} /></View>
                        <View style={styles.bizInfo}>
...
const styles = StyleSheet.create({
    ...
    cardImageContainerLarge: { height: 180, overflow: 'hidden' },
    bizImgLarge: { width: '100%', height: '100%' },
    bizInfo: { padding: 12 },
    bizName: { fontSize: 15, fontWeight: '900', color: '#000' },
    bizSubtitle: { fontSize: 11, color: '#777', marginTop: 4, lineHeight: 16 },
});

export default GenericGridSection;
